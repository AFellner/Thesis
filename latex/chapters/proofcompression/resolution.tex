...
Clauses represent formulas by interpreting it as the disjunction of its literals.
Sets of clauses represent formulas by interpreting them as the conjunction of the interpreted clauses.
...
However if it is possible to resolve a clause with itself, this means that the clause contains both the positive and negative version of a variable and is therefore tautological (i.e. trivially satisfiable).
...
Therefore the choice of literal to resolve on is not of interest to us.
...
The labeling function $\mathcal{L}$ maps nodes to clauses and edges to literals.
...
Therefore the underlying structure of a proof is really a directed acyclic graph (DAG) and not simply a tree. 
...
Consider the propositional logic formula $\Phi := (x_1 \vee x_2 \vee \neg x_3) \wedge (x_1 \vee x_2) \wedge (x_1 \vee x_3) \wedge (\neg x_1)$.
...
\input{chapters/proofcompression/figures/resolutionexample}
