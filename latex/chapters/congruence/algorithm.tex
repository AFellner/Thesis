...
After the attempt to insert an edge to the graph via the \texttt{lazy\_insert} and subsequently the \texttt{insert} method there is always a path between the arguments.
...
Let the induction hypothesis be, that for every term $s$ such that $|[r(s)]| \leq n$, it is the case that $r(s) = r(t)$ if and only if there is a path between $s$ and $t$ in the congruence graph.
...
This results in $O(n \log(n))$ edge reversing steps, where $n$ is the number terms in the input equation set, which can be shown just like in the proof of Proposition \ref{prop:runtime}.
