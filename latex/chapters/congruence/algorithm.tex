...
The idea of lazy edge insertion is have as much information as possible when explanations for edges, corresponding to equalities that are due to the compatible axiom, are searched.
Not only could the explanations be shorter when more information is available, such deduced equalities might even be part of the input and would not have to be deduced at all.
It is important to keep track of the order of edge input, because explanations might depend on other edges to be available in the graph.
Example \ref{example:lazy} shows how this technique can help finding short explanations.

{\color{blue} example not done yet; I am not sure whether it's better to put it here or in the congruence graph section}
