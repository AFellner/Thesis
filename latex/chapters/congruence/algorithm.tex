...
For our purpose, the input equations together with deduced equalities have to be stored in a data structure that supports the production of explanations.
We support two different such data structures.
Both structures store equations in a labeled graph, that we call a congruence graph.
The nodes represent terms and an edge between two nodes denotes that the represented terms are congruent w.r.t. the set of input equations.
...
For terms $s$, $t$ with equal representatives, there is a sequence of calls to union.
Every union is called from \texttt{merge}, which calls \texttt{lazy\_insert} right after union with the same argument.
The medthod \texttt{lazy\_insert} eventually adds an edge between the respective terms.
Therefore the sequence of unions corresponds to the path between $s$ and $t$.
...
Therefore the statement \texttt{inputEqs(explain(s,t,g),g)} returns an explanation for $E \models s \thickapprox t$.
...
The weight for an edge is the number of input equalities used to derive the equality.
This number is one for input equalities and the size of the explanation for deduced equalities.
Edges are added to the graph, regardless whether there already exists a path between the two edges.
Therefore there is a choice which path the explain method returns.
To produce short explanations, the shortest path is returned.
To find the shortest path between two nodes, Dijkstra's shortest path algorithm has been implemented.
Dijkstra's shortest path algorithm does not have optimal asymptotic runtime, \cite{TODO} reports of an linear algorithm for the single source shortest path problem, but \cite{TODO} shows that the algorithm behaves well in practice.
Dijkstra's algorithm works in a greedy way, always choosing the best edge locally and never backtracking previously made decisions.
