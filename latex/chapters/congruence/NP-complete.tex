...
Our proof of NP-completeness reduces the problem of deciding the satisfiability of a propositional logic formula in conjunctive normal form to the short explanation decision problem.
...
First we define how to translate propositional formulae into sets of equations.
...
Therefore replacing $Assignment$ with $Assignment' := Assignment \setminus \{(\hat{x}_2,\top_2)\} \cup \{(\hat{x}_2,\bot_1)\}$ in $E_{\Phi}$ leads to another explanation of $(\hat{c}_1,\hat{c}_4)$ of equal size.
...
Using the transitivity axiom it follows that $T_{ij} \models \hat{c}_i \thickapprox \hat{c}_{i+1}$.
...
Using the transitivity axiom it follows that $E \models \hat{c}_1 \thickapprox \hat{c}_{n+1}$.
...
Therefore in order to satisfy $E \models \hat{c}_1 \thickapprox t$ such that $t$ is neither the constant $\hat{c}_1$, nor some term $t_1(\hat{x}_j), f_1(\hat{x}_j)$, it is necessary that $E \models t_1(\hat{x}_j) \thickapprox t_1(\top_j)$ and $(\hat{c}_1, t_1(\hat{x}_j)) \in E$ or $E \models f_1(\hat{x}_j) \thickapprox f_1(\bot_j)$ and $(f_1(\bot_j),\hat{c}_2) \in E$ for some $j$.
...
We reduce the problem of deciding the satisfiability of a propositional logic formula (better known as SAT) to the short path decision problem.
...
Let $E = \bigcup_i^n E_i$ then from Lemma \ref{lemma:charexpl} and the transitivity of congruence relations follows $E \models c_1 \thickapprox c_{n+m+1}$.
