...
The subproof corresponding to the explanation $\{(a,b)\}$, that is displayed in Figure \ref{fig:short_expl_proof}, is created and $O_1$ is replaced by this subproof $N_1$.
When iterating over node $O_2$, the algorithm detects that the pivot literal $t_a = a$ is not present $N_1$ and \texttt{fixNode} replaces $O_2$ by $N_1$.
...
The resultion proof is displayed in Figure \ref{fig:compressproof_2}.
The resulting proof has proof length 11 which is shorter than the original one, even though the replaced subproof is bigger.
